...
def main():
    ...
    with gr.Blocks(title="ChatGPT 学术优化", theme=set_theme, analytics_enabled=False, css=advanced_css) as demo:
        ...
        with gr_L1():
            with gr_L2(scale=2):
                chatbot = gr.Chatbot(height=CHATBOT_HEIGHT)
                history = gr.State([])
            with gr_L2(scale=1):
                with gr.Accordion("输入区", open=True) as area_input_primary:
                    with gr.Row():
                        txt = gr.Textbox(show_label=False, placeholder="Input question here.", container=False)
                    with gr.Row():
                        submitBtn = gr.Button("提交", variant="primary")
                    with gr.Row():
                        resetBtn = gr.Button("重置", variant="secondary", size="sm")
                        stopBtn = gr.Button("停止", variant="secondary", size="sm")
                        clearBtn = gr.Button("清除", variant="secondary", visible=False, size="sm")
                    with gr.Row():
                ...
                with gr.Accordion("备选输入区", open=True, visible=False) as area_input_secondary:
                    with gr.Row():
                        txt2 = gr.Textbox(show_label=False, placeholder="Input question here.", label="输入区2", container=False)
                    with gr.Row():
                        submitBtn2 = gr.Button("提交", variant="primary")
                    with gr.Row():
                        resetBtn2 = gr.Button("重置", variant="secondary", size="sm")
                        stopBtn2 = gr.Button("停止", variant="secondary", size="sm")
                        clearBtn2 = gr.Button("清除", variant="secondary", visible=False, size="sm")
        # 功能区显示开关与功能区的互动
    ...
    import configparser
    # 全局快捷键（macOS/Apple Silicon 上 keyboard 库可能导致 bus error，故跳过）
    import threading
    import platform
    _use_keyboard_hotkeys = (platform.system() != "Darwin")
    ...
    class DesktopPet(QWidget, QObject):
        def __init__(self, config):
            ...
            # 监听全局快捷键的线程（macOS 上跳过以避免 keyboard 库导致的 bus error）
            if not _use_keyboard_hotkeys:
                print("提示：macOS 上已禁用全局快捷键（可避免崩溃），请使用右键菜单打开功能。")
            if _use_keyboard_hotkeys:
        ...
        def init_ui(self):
            ...
            #宠物信息（路径需兼容 macOS/Linux，将反斜杠转为正斜杠）
            icon_path = self.config["Pet"]["PET_ICON"].replace("\\", "/")
            self.pet_movie = QMovie(icon_path)
        ...
        # 由于 keyboard 库的 add_hotkey 会阻塞，在单独线程中运行。macOS 上跳过（易触发 bus error）。
