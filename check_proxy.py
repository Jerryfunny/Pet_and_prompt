def check_proxy(proxies):
    ...
    result = f"代理配置 {proxies_https}"
    # 尝试多个 IP 查询服务，避免单一服务频率限制
    apis = [
        ("https://ipapi.co/json/", lambda d: d.get('country_name') if not d.get('error') else None),
        ("https://ip-api.com/json/?fields=country", lambda d: d.get('country') if d.get('status') != 'fail' else None),
    ]
    for url, get_country in apis:
        try:
            response = requests.get(url, proxies=proxies, timeout=4)
            data = response.json()
            country = get_country(data)
            if country:
                result = f"{result}, 代理所在地：{country}"
                break
        except Exception:
            continue
    else:
        result = f"{result}, 代理所在地：未知（IP 查询受限或超时）"
    print(result)
    return result
